    ...
    if (heatmapSource === 'csv' && showCsvPoints) {
      heatmapData = csvData
        .map(row => {
          ...
          // Use 5 for 'yes', 0 for 'no' - increased contrast
          const weight = score === 'yes' ? 5 : 0;
      ...
      heatmapData = greenScoreData
        .map(row => {
          ...
          // Amplify the score weighting for more noticeable effect
          // Score 1 = weight 1, Score 10 = weight 20 (doubled range)
      ...
      <LoadScript googleMapsApiKey="AIzaSyCp6YMW24ocLyfToDWWFs_FmUuN7AwVm4c" libraries={["visualization"]}>
        ...
        >
          ...
          {showGreenScore && greenScoreData.map((row, i) => {
            ...
            const scale = 6 + ((score - 1) / 9) * 10; // 1:6, 10:24
            return (
              <Marker
                key={`green-score-${i}`}
                position={{ lat, lng }}
          ...
          <HeatmapLayer 
            ...
            options={{ 
              radius: 10 + (heatmapRadiusLevel - 1) * 10, 
              opacity: 0.8,
              gradient: [
                'rgba(0, 255, 255, 0)',
                'rgba(0, 255, 255, 1)',
